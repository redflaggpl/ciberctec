...
/**
 * @author Marosdee Uma, 
 * 	website:: www.danya-reload.com @ danya0365[at]live.com, 
 * 	danya0365[at]gmail.com, danya-planemo[at]live.com
 * @copyright 2009 
 * @version 1.5
 */
...
class Agenda
{
	...
	* @var array
	*/	
	private $colorArray = array('#D4D4FF','#FFD4FF','#FFD4D4','#FFFFD4','#D4FFD4','#D4FFFF','#E1E1E1');
	...
	* @var array
	*/	
	private $dayOfWeekTitle = array('Domingo','Lunes','Martes','Mi&eacute;rcoles','Jueves','Viernes','S&aacute;bado');
...
HTML;
	...
	*/
	public function calendarMonth( $m = '', $y = '', $inYearCalendar = false)
	{
		...
		$backNavigation = ( ! $inYearCalendar ) ? "<td class='calendar-header'><a href='index2.php?com=agenda&m={$backMonth}&y={$backYear}' onclick='getCalendarMonth($backMonth,$backYear); return false;'> &lt;&lt; </a></td>" : '&nbsp;'; 

		$nextNavigation = ( ! $inYearCalendar ) ? "<td class='calendar-header'><a href='index2.php?com=agenda&m={$nextMonth}&y={$nextYear}' onclick='getCalendarMonth($nextMonth,$nextYear); return false;'> &gt;&gt; </a></td>" : '&nbsp;';
		...
		return <<<HTML
			...
			<table width="{$this->defaultTableWidth}" class="calendartable">
				...
				<tr>
					<td colspan='7' class="calendar-today">
						<a href='index2.php?com=agenda&m={$this->todayMonth}&y={$this->todayYear}'> Today </a>
					</td>
				</tr>
			</table>\n
HTML;
...
HTML;
	...
	*/
	protected function _dayFormat($day)
	{
		...
		$cell = "<td $class $bg>";
		$cell .= "<a href='index2.php?com=agenda&do=day&m=
				{$this->todayMonth}&y={$this->todayYear}&d=$day'>$dayFormat</a>";
		$cell .= $this->_getEvents($day)."</td>\n";
	...
	*/
	...
	public function _getEvents($day)
	{
		global $uid;
		
		$db = new DB();
		$sql  = "SELECT id, titulo, descrip, fechaini, funcionario_id, tipo, ";
		$sql .= "fechafin, lugar ";
		$sql .= " FROM eventos ";
		$sql .= "WHERE DATE(fechaini) <= '$this->defaultYear-$this->defaultMonth-$day' ";
		$sql .= "AND DATE(fechafin) >= '$this->defaultYear-$this->defaultMonth-$day' ";
		$sql .= "AND funcionario_id = $uid";
		$res = $db->query($sql);
		$data = "";
		while($line = $db->fetchNextObject($res))
		{
			$data .= "<p>&raquo; <a onClick=\"getEvent('$line->id');\">$line->titulo</a> </p>";
		}
		return $data;
	}
}
